...
check(total >= 50, `layout: a busy room (${total} swallowable props)`);
check(eng.props.some((p) => p.desc.fixture), 'layout: has a bolted fixture');
check(eng.props.some((p) => p.desc.walker), 'layout: has wind-up walkers');
check(eng.props.some((p) => p.desc.pinata), 'layout: has a piñata');
...
while (eng.remaining() > 0 && steps < MAX_STEPS) {
  ...
  for (const p of eng.props) {
    ...
    if (p.supportId && p.state !== S.GONE) {
      ...
      if (sup && sup.desc.topR && (sup.desc.fixture || sup.desc.footInR > r || !targetFor(sup, r))) {
  ...
  if (goto_) {
    ...
    // nothing fits yet: park under the smallest prop and wait for a wedge
    let smallest = null;
    for (const p of eng.props) {
  ...
  for (const ev of eng.events) {
    seen.add(ev.type);
    if (ev.type === 'pinata') {
      // simulate main.js: burst the contents into the room
      const minis = [];
      for (let i = 0; i < 9; i++) {
        minis.push(i % 3 === 0
...
console.log(`    bot cleared ${eng.props.filter((p) => p.state === S.GONE).length} props in ${(steps / 60).toFixed(0)}s sim, hole r=${eng.hole.r.toFixed(2)}, launches=${launches}`);
check(eng.remaining() === 0, 'clear: the whole room (incl. piñata contents) can be swallowed');
...
check(eng.props.filter((p) => p.desc.fixture).every((p) => p.state !== S.GONE),
  'clear: fixtures stayed bolted to the wall');
